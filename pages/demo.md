...
`ScatterStructurePlot`を使うと、Evidence 標準の scatter plot と同じ見た目で描画したグラフに、マウスオーバーした際のポップアップとして化学構造を表示できます。例えば以下では、`potency` と `logp` を縦横軸に、`smiles` 列を構造描画に利用しています。`backgroundColor` や `pointColor` を指定すると、図の雰囲気を好きな色にまとめられます。
